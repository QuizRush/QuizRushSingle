...
@export var health_changed_label = preload("res://health_changed_label.tscn")
@export var damage_color : Color = Color.DARK_RED
@export var heal_color : Color = Color.DARK_GREEN

#add_child(health_changed_label)

# Called when the node enters the scene tree for the first time.
func _ready():
	Global.connect("on_health_changed", on_signal_health_changed)
